...
public class FloatingTextComponent : MonoBehaviour {
    ...
    public void Initialize ( ScriptableText sct, Vector3 pos, string sctText, Camera targetCam ) {
        ...
        _animDuration = sct.AnimCurveX.keys[sct.AnimCurveX.length - 1].time >= sct.AnimCurveY.keys[sct.AnimCurveY.length - 1].time ? sct.AnimCurveX.keys[sct.AnimCurveX.length - 1].time : sct.AnimCurveY.keys[sct.AnimCurveY.length - 1].time;
