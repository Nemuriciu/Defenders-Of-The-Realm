...
public class Healthbar : MonoBehaviour {
	...
	public Damager healthLink;				//A path to the Health filed;			
	...
	private void Start () {
        ...
        canvasGroup.alpha = alphaSettings.fullAlpha;
	...
	private void FixedUpdate () {
		...
		healthVolume.fillAmount =  healthLink.health/defaultHealth;
	...
	private void Update() {
		...
		if(showHealthInfo)
			healthInfo.text = healthLink.health + " / " +defaultHealth;
		else
			healthInfo.text = "";

        if(lastHealth != healthLink.health) {
            rate = Time.time + alphaSettings.onHit.duration;
            lastHealth = healthLink.health;
        }

        if (!OutDistance() && IsVisible()) {
            if (healthLink.health <= 0) {
            ...
            else if (healthLink.health == defaultHealth)
                canvasGroup.alpha = alphaSettings.fullFadeSpeed > 0 ? Mathf.MoveTowards(canvasGroup.alpha, alphaSettings.fullAlpha, alphaSettings.fullFadeSpeed) : alphaSettings.fullAlpha;
            else {
                if (rate > Time.time)
		...
		switch (healthInfoAlignment) {
			case HealthInfoAlignment.top:
				healthInfo.rectTransform.anchoredPosition = healthInfoPosition;
				break;
			case HealthInfoAlignment.center:
				healthInfo.rectTransform.anchoredPosition = Vector2.zero;
				break;
			case HealthInfoAlignment.bottom:
				break;
			default:
				healthInfo.rectTransform.anchoredPosition = -healthInfoPosition;
				break;
    ...
    private bool OutDistance() {
        return DrawOFFDistance && camDistance > drawDistance;
    }
