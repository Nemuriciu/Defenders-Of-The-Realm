public static class Stats {
    ...
    public static int PlayerGold {
        get => _playerGold;
        set => _playerGold = _playerGold + value > MaxGold ? MaxGold : value;
    }

    private static int _playerEnergy = MaxEnergy;
    public static int PlayerEnergy {
        //get => _playerEnergy;
        set => _playerEnergy = _playerEnergy + value > MaxEnergy ? MaxEnergy : value;
    }
    
    private static int _artefactHealth;
    public static int ArtefactHealth {
        get => _artefactHealth;
        set => _artefactHealth = _artefactHealth + value < 0 ? 0 : value;
    }
}